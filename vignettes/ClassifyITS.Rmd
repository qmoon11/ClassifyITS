...
## Potential masked warnings

Depending on the version of R and the packages installed, you may see warnings about masked functions when running ClassifyITS. These are generally not a cause for concern as they indicate that certain functions from ClassifyITS have the same names as functions in other packages (usually data.table and Biostrings). The warning is just letting you know which version of the function will be used when called. These warnings can be safely ignored as they do not indicate any actual problems with the functionality of ClassifyITS.
